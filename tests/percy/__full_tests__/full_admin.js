...
describe('Full regression test suite for Admin', () => {
  ...
  it(
    'Admin can create a group and add Ally as admin',
    async () => {
      try {
        ...
        await page.type('#edit-field-official-organization-name-0-value', `Official name of ${TEST_GROUP_PATH}`);
        await page.select('#edit-field-migration-status', 'Complete')
        await page.type('#edit-field-summary-0-value', `This is a test summary for the ${TEST_GROUP_PATH}`);
        ...
        await page.goto(`${HOME_PAGE}/${TEST_GROUP_PATH}/content/add/group_membership?destination=/${TEST_GROUP_PATH}/members`);
        text_content = await page.evaluate(() => document.querySelector('.page-title').textContent);
  ...
  it(
    ...
    async () => {
      try {
        ...
        await page.goto(`${HOME_PAGE}/${TEST_GROUP_PATH}/content/create/group_node:page`, { waitUntil: 'networkidle2' });
