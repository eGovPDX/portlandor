...
exports.masqueradeAs = async function (email, page, HOME_PAGE) {
  ...
  await page.goto(
    `${HOME_PAGE}/admin/people?user=${encodedEmail}&status=All&role=All&permission=All`
  );
  var link = await page.evaluate(() =>
    document.querySelector('li.masquerade a').getAttribute('href')
  );
...
exports.unmasquerade = async function (page, HOME_PAGE) {
  ...
  var link = await page.evaluate(() =>
    document
      .querySelector('div.toolbar-tab a[href^="/unmasquerade')
      .getAttribute('href')
  );
  await page.goto(`${HOME_PAGE}${link}`);
};

exports.removeAlert = async function (page) {
  // Remove sitewide alert
  await page.evaluate(() =>
    document.querySelector('#block-cloudy-views-block-alerts-block-1').remove()
  );
};
...
exports.ContentTester = {
  ...
  runTest: async function () {
    ...
    if (contentUrl.indexOf(this.homepageUrl) != 0)
      contentUrl = this.homepageUrl + contentUrl;
  ...
  verifyFieldLabels: async function () {
    ...
    text_content = await this.page.evaluate((addFormId) => {
  ...
  submitForm: async function () {
    ...
    let contentUrl = await this.page.evaluate(() => {
      ...
      if (document.querySelector('div.messages__content a'))
        return document
          .querySelector('div.messages__content a')
          .getAttribute('href');
      return;
    });
  ...
  deleteContent: async function (deleteContentUrl) {
    ...
    let deleteFormId = `#${this.entityType}-${this.contentType.replace(
      '_',
      '-'
    )}-delete-form`;
