...
namespace Drupal\media_embed_field;
...
abstract class ProviderPluginBase extends PluginBase implements ProviderPluginInterface, ContainerFactoryPluginInterface {
   ...
   * @var string
   */
  protected $thumbsDirectory = 'public://media_thumbnails';
  ...
  public function __construct($configuration, $plugin_id, $plugin_definition, ClientInterface $http_client) {
    ...
    if (!static::isApplicable($configuration['input'])) {
      throw new \Exception('Tried to create a media provider plugin with invalid input.');
    }
    $this->input = $configuration['input'];
    $this->mediaId = $this->getIdFromInput($configuration['input']);
  ...
  public function getLocalThumbnailUri() {
    return $this->thumbsDirectory . '/' . $this->getMediaId() . '.jpg';
  }
  ...
  public function getName() {
    return $this->t('@provider Media (@id)', ['@provider' => $this->getPluginDefinition()['title'], '@id' => $this->getMediaId()]);
  }
