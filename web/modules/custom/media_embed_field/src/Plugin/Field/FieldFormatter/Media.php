...
/**
 * Plugin implementation of the media field formatter.
 *
 * @FieldFormatter(
 *   id = "media_embed_field_media",
 ...
 * )
 */
class Media extends FormatterBase implements ContainerFactoryPluginInterface {
   ...
   *   The media embed provider manager.
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    foreach ($items as $delta => $item) {
      ...
      if (!$provider) {
        $element[$delta] = ['#theme' => 'media_embed_field_missing_provider'];
      }
      else {
        $autoplay = $this->currentUser->hasPermission('never autoplay media videos') ? FALSE : $this->getSetting('autoplay');
        ...
        $element[$delta] = [
          '#type' => 'container',
          '#attributes' => ['class' => [Html::cleanCssIdentifier(sprintf('media-embed-field-provider-%s', $provider->getPluginId()))]],
          'children' => $element[$delta],
        ];

        // For responsive media, wrap each field item in it's own container.
        if ($this->getSetting('responsive')) {
          $element[$delta]['#attached']['library'][] = 'media_embed_field/responsive-media';
          $element[$delta]['#attributes']['class'][] = 'media-embed-field-responsive-media';
        }
      }
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    ...
    $elements['autoplay'] = [
      ...
      '#type' => 'checkbox',
      '#description' => $this->t('Autoplay the videos for users without the "never autoplay media videos" permission. Roles with this permission will bypass this setting.'),
      '#default_value' => $this->getSetting('autoplay'),
    ];
    $elements['responsive'] = [
      '#title' => $this->t('Responsive Media'),
      '#type' => 'checkbox',
      '#description' => $this->t("Make the media fill the width of it's container, adjusting to the size of the user's screen."),
  ...
  public function settingsSummary() {
    ...
    $summary[] = $this->t('Embedded Media (@dimensions@autoplay).', [
  ...
  /**
   * Get an instance of the Media field formatter plugin.
   *
   * This is useful because there is a lot of overlap to the configuration and
   * display of a media in a WYSIWYG and configuring a field formatter. We
  ...
  public static function mockInstance($settings) {
    return \Drupal::service('plugin.manager.field.formatter')->createInstance('media_embed_field_media', [
