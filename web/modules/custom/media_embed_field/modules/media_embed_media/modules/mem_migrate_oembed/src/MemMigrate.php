...
namespace Drupal\mem_migrate_oembed;
...
class MemMigrate {
  ...
  public function migrate() {
    ...
    foreach ($this->configFactory->listAll('media.type.') as $media_type) {
      ...
      if ($media_type->get('source') !== 'media_embed_field') {
        continue;
      }

      $media_type->set('source', 'oembed:media');
    ...
    foreach ($this->configFactory->listAll('field.storage.media') as $field_storage) {
      ...
      if ($field_storage->get('type') !== 'media_embed_field') {
        continue;
      }
    ...
    foreach ($this->configFactory->listAll('field.field.media') as $field_config) {
      ...
      if ($field_config->get('field_type') !== 'media_embed_field') {
        continue;
      }
    ...
    foreach ($this->configFactory->listAll('core.entity_view_display.media') as $entity_view_display) {
      ...
      foreach ($fields as $field) {
        if ($entity_view_display->get("content.$field.type") !== 'media_embed_field_media') {
          continue;
        }
    ...
    foreach ($this->configFactory->listAll('core.entity_form_display.media') as $entity_form_display) {
      ...
      foreach ($fields as $field) {
        if ($entity_form_display->get("content.$field.type") !== 'media_embed_field_textfield') {
          continue;
        }
