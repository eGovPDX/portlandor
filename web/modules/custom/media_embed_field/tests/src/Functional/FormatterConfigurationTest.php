...
class FormatterConfigurationTest extends BrowserTestBase {
   ...
   */
  public function testMediaConfirmationForm() {
    // Test the settings form and summaries for the media formatter.
    $this->setFormatter('media_embed_field_media');
    $this->assertSession()->pageTextContains('Embedded Media (Responsive, autoplaying).');
    ...
    $this->assertSession()->pageTextContains('Embedded Media (100x100).');

    // Test the image formatter.
    $this->setFormatter('media_embed_field_thumbnail');
    $this->assertSession()->pageTextContains('Media thumbnail (no image style).');
    $this->updateFormatterSettings([
      'image_style' => 'thumbnail',
      'link_image_to' => Thumbnail::LINK_CONTENT,
    ]);
    $this->assertSession()->pageTextContains('Media thumbnail (thumbnail, linked to content).');
    $this->updateFormatterSettings([
      'image_style' => 'medium',
      'link_image_to' => Thumbnail::LINK_PROVIDER,
    ]);
    $this->assertSession()->pageTextContains('Media thumbnail (medium, linked to provider).');

    $this->setFormatter('media_embed_field_colorbox');
