...
export class AddressWrapper {
  constructor(addressField) {
    const selector = `.field--type-address.field--name-${addressField.replace(
      /_/g,
      "-"
    )}`;
    ...
    $(document).ajaxComplete((event, xhr, settings) => {
      ...
      if (
        ...
        settings.extraData._triggering_element_name ===
          `${addressField}[0][address][country_code]`
      ) {
        this.fillFields();
      }
    });
      ...
      if (fieldMap.hasOwnProperty(component)) {
        ...
        if (component === "State") {
          this.address[component] =
            stateMap[this.address[component]] || this.address[component];
        }
        this.addressField
          .find(`.${addressClass}`)
          .val(this.address[component])
          .change();
      }
    }
  }
