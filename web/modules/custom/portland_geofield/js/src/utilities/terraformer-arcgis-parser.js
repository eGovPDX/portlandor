...
function parse(arcgis, options) {
  ...
  if (
    ...
    (arcgis.spatialReference.wkid === 3857 ||
      arcgis.spatialReference.wkid === 102100)
  ) {
  ...
  if (arcgis.compressedGeometry || arcgis.geometry || arcgis.attributes) {
    ...
    if (arcgis.compressedGeometry) {
      arcgis.geometry = {
        paths: [decompressGeometry(arcgis.compressedGeometry)]
      };
    }
    ...
    if (arcgis.attributes) {
      geojson.id =
        arcgis.attributes[options.idAttribute] ||
