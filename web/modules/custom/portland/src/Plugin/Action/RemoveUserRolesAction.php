...
/**
 * Allows non-admin users to remove specified sitewide roles from a user. Available roles are selected in the VBO preconfiguration form.
 */
#[Action(
  id: 'portland_remove_user_roles',
  label: new TranslatableMarkup('Remove sitewide roles from a user (custom action)'),
  type: 'user',
)]
class RemoveUserRolesAction extends ViewsBulkOperationsActionBase implements PluginFormInterface, ContainerFactoryPluginInterface {
  /**
   * Entity type manager.
   *
  ...
  public function execute(?UserInterface $user = NULL) {
    ...
    return $this->t('Removed roles from the user.');
  }
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    $form['roles'] = [
      '#title' => $this->t('Select the role(s) to remove from these users'),
      '#type' => 'checkboxes',
  ...
  public function buildPreConfigurationForm(array $form, array $values, FormStateInterface $form_state): array {
    ...
    $form['available_roles'] = [
      ...
      '#description' => $this->t('Only select roles that those with access to this view should have access to remove from other users.'),
    ];
