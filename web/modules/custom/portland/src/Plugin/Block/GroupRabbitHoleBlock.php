...
class GroupRabbitHoleBlock extends BlockBase {
  ...
  private static function buildRenderArray($node_latest_revision, $node_current_revision, $node_default_revision) {
    ...
    $nid = $node_current_revision->id->value;
    $node_latest_vid = $node_latest_revision->revision_id->value;
    $node_current_vid = $node_current_revision->revision_id->value;
    $node_current_is_archived = ($node_current_revision->moderation_state->value == 'archived');
    $node_default_vid = $node_default_revision->revision_id->value;
    ...
    dump($node_latest_vid, $node_default_vid);
  ...
  private static function loadLatestRevision($nid, $langcode) {
    $latestRevisionId = \Drupal::entityTypeManager()->getStorage('group')->getLatestTranslationAffectedRevisionId($nid, $langcode);
