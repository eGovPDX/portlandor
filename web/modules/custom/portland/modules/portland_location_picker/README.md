...
To support the widest array of applicaitons, by default all sub-elements are enabled and none are required. In practice, location_address and location_lat should usually be marked required in the element configuration panel. Other sub-elements can be disabled if not needed. A common default pattern was not coded into the custom element plugin, because overriding that pattern would require using arcane YAML code in the custom properties field. Though this requires more initial configuration, it consists of checking boxes in the UI, which is much simpler than figuring out the YAML custom properties.

When fields are marked as required in the UI, instead of the custom code for the widget, they are only required when not disabled/hidden by conditional logic. If field requirements are hard coded in the module, Drupal still thinks they're required even if they're not enabled/visible. This is one of the reasons no fields are required by default in the module code.
