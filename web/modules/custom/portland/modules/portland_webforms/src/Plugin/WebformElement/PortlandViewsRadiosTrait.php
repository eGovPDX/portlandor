...
trait PortlandViewsRadiosTrait {
  private const CACHE_PREFIX = 'portland_webforms:views_radios:';
  ...
  /**
   * Handle when the format is a field from a view, e.g. `webform_submission:values:my_radios:cell_2`.
    ...
    if (!empty($format) && $format !== 'value' && $format !== 'raw') {
      ...
      if ($view && $view->access($display_id)) {
        ...
        if (array_key_exists($format, $fields)) {
          ...
          $value_row = array_find($field_data, fn($row) => ($row[$value_field_id] ?? NULL) === $value);
          if ($value_row) {
