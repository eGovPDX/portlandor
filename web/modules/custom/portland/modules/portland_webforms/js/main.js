// main.js
(function ($, Drupal, drupalSettings, L) {
    Drupal.behaviors.portland_location_picker = {
      attach: function (context) {
        $(once('portland-location-picker', '.portland-location-picker', context)).each(function () {
          const apiKey = drupalSettings.portlandmaps_api_key;
          const elementId = drupalSettings.webform.portland_location_picker.element_id;
          const settings = drupalSettings.webform.portland_location_picker;
  
          const mapInitializer = new MapInitializer('location_map_container', settings);
  
          // Example of using the Api module
          async function getReverseGeocode(lat, lng) {
            try {
              const data = await Api.reverseGeocode(lat, lng, apiKey);
              console.log(data);
            } catch (error) {
              console.error('Error fetching reverse geocode data:', error);
            }
          }
  
          // Other initialization code, event listeners, etc.
        });
      }
    };
  })(jQuery, Drupal, drupalSettings, L);
  