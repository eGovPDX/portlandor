...
L.GeometryUtil = {
    ...
    distance: function (map, latlngA, latlngB) {
        //console.log(latlngA);
        //console.log(map.latLngToLayerPoint(latlngA));
        //console.log(map.project(latlngA));
        //console.log(map.latLngToLayerPoint(latlngA).distanceTo(map.latLngToLayerPoint(latlngB)));
        return map.latLngToLayerPoint(latlngA).distanceTo(map.latLngToLayerPoint(latlngB));
    },

    distance2: function (latlng1, latlng2) {
    ...
    */
    ...
    /**
        Returns the closest layer to latlng among a list of layers.
        @param {L.Map} map
        @param {Array<L.ILayer>} layers = layer group
        @param {L.LatLng} latlng
        @returns {object} with layer, latlng and distance or {null} if list is empty;
    */
    closestLayer2: function (map, layers, latlng) {
        ...
        layers.eachLayer(function (layer) {
            ...
            if (typeof layer.getLatLng == 'function') {
                ll = layer.getLatLng();
                distance = L.GeometryUtil.distance2(latlng, ll);
            }
            else {
                ll = L.GeometryUtil.closest(map, layer, latlng);
                if (ll) distance = ll.distance;  // Can return null if layer has no points.
                ...
                result.unshift({layer: layer, latlng: ll, distance: distance});
